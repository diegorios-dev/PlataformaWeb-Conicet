...
export default function HistogramaNieve() {
  ...
  const fetchData = useCallback(() => {
    ...
    if (periodo === "dia") {
      return getHistogramaNieve(periodo, year, month);
    } else if (periodo === "mes") {
      return getHistogramaNieve(periodo, year, null);
    } else {
      return getHistogramaNieve(periodo, null, null);
    }
  }, [periodo, year, month]);
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      // Agregar título
      pdf.setFontSize(18);
      pdf.text('Histograma de Nieve', 148, 15, { align: 'center' });
      ...
      // Guardar PDF
      const fileName = `histograma-nieve-${periodo}-${year}${periodo === 'dia' ? `-${month}` : ''}.pdf`;
      pdf.save(fileName);
  ...
  return (
    <div className="w-4/5 mx-auto my-10">
      ...
      <h2 className="text-center text-2xl font-semibold mb-6">Histograma de Nieve</h2>
        ...
        <div ref={chartRef} className="bg-white rounded-lg shadow p-6">
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={data}>
              ...
              <Tooltip />
              <Bar dataKey="value" fill="#81D4FA" radius={[5, 5, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
